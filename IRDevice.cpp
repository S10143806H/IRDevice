...
void IRDevice::sendSONY(unsigned long data, uint16_t len) {
    ...
    for (index = 0; index < MAX_LOG_WAVFORM_SIZE; index++) {
        Log0[index] = ConvertToCarrierCycle(IR_Protocol->log0Buf[index], IR_DataStruct.carrierFreq);
        Log1[index] = ConvertToCarrierCycle(IR_Protocol->log1Buf[index], IR_DataStruct.carrierFreq);
        // printf("Log0[%d]=%x log1[%d]=%x\n\r", index, IR_Protocol->log0Buf[index], index, IR_Protocol->log1Buf[index]);
    }
    ...
    for (index = 0; index < 2; index++) {
        ...
        printf("header buf is %x\n\r", IR_Protocol->headerBuf[index]);
        bufLen++;
    }
...
const IR_ProtocolTypeDef_RC5 RC5_PROTOCOL =
    {
        36000,                               /* Carrier freqency */
        2,                                   /* headerLen */
        {PULSE_LOW | 889, PULSE_HIGH | 889}, /* headerBuf unit: us*/
        {PULSE_HIGH | 889, PULSE_LOW | 889}, /* log0Buf */
        {PULSE_LOW | 889, PULSE_HIGH | 889}, /* log1Buf */
};

void IRDevice::sendRC5(unsigned long data, uint16_t len) {
    /* declare vairables */
    u32 tx_count = 0;
    const u8 tx_thres = 1;
    uint16_t index = 0;
    ...
    uint32_t inputTime = 0;
    int nbits = len;
    IR_ProtocolTypeDef *IR_Protocol = (IR_ProtocolTypeDef *)(&RC5_PROTOCOL);

    IR_DataStruct.carrierFreq = IR_InitStruct.IR_Freq;
    IR_DataStruct.codeLen = len;

    /* Encode logic 1 and logic 0 */
    for (index = 0; index < MAX_LOG_WAVFORM_SIZE; index++) {
        Log0[index] = ConvertToCarrierCycle(IR_Protocol->log0Buf[index], IR_DataStruct.carrierFreq);
        Log1[index] = ConvertToCarrierCycle(IR_Protocol->log1Buf[index], IR_DataStruct.carrierFreq);
        // printf("Log0[%d]=%x log1[%d]=%x\n\r", index, IR_Protocol->log0Buf[index], index, IR_Protocol->log1Buf[index]);
    ...
    /* Encode toggle bit */
    for (index = 0; index < 2; index++) {
        IR_DataStruct.irBuf[index] = ConvertToCarrierCycle(IR_Protocol->log0Buf[index], IR_DataStruct.carrierFreq);
        // printf("header buf is %x\n\r", IR_Protocol->headerBuf[index]);
        bufLen += 1;
    }
    printf("header + toggle length: %d\n\r", bufLen);
    ...
    tx_count += bufLen;
    printf("--------------Start sending---------------\r\n");
    printf("buffer length - tx_count: %d\r\n", IR_DataStruct.bufLen - tx_count);
    while ((IR_DataStruct.bufLen - tx_count) > 0) {
        while (IR_GetTxFIFOFreeLen(IR_DEV) < tx_thres) {
            printf("task yield\r\n");
            taskYIELD();
